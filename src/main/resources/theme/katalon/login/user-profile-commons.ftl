<#macro userProfileFormFields>
    ...
    <#list profile.attributes as attribute>
        ...
        <#else>
                ...
                <#if currentGroup != "">
                    ...
                    >
                        ...
                        <div class="${properties.kcContentWrapperClass!}">
                            <label id="header-${attribute.group.name}" class="form-label fw-bold">${groupHeaderText}</label>
                        </div>
                        ...
                        <#if groupDisplayDescription != "">
                            ...
                            <div class="${properties.kcLabelWrapperClass!}">
                                <label id="description-${group.name}" class="form-text text-muted">${groupDescriptionText}</label>
                            </div>
                        </#if>
                    </div>
                </#if>
            ...
            <div class="${properties.kcFormGroupClass!} mb-3 col-12">
                <div class="${properties.kcLabelWrapperClass!}">
                    <label for="${attribute.name}" class="form-label ${properties.kcLabelClass!}">
                        ${advancedMsg(attribute.displayName!'')}
                        <#if attribute.required><span class="text-danger">*</span></#if>
                    </label>
                </div>
                <div class="${properties.kcInputWrapperClass!}">
                    <#if attribute.annotations.inputHelperTextBefore??>
                        <div class="${properties.kcInputHelperTextBeforeClass!} form-text text-muted mb-2" id="form-help-text-before-${attribute.name}" aria-live="polite">
                            ${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextBefore))?no_esc}
                        </div>
                    </#if>
                    <@inputFieldByType attribute=attribute/>
                    <#if messagesPerField.existsError('${attribute.name}')>
                        <div id="input-error-${attribute.name}" class="${properties.kcInputErrorMessageClass!} invalid-feedback d-block" aria-live="polite">
                            ${kcSanitize(messagesPerField.get('${attribute.name}'))?no_esc}
                        </div>
                    </#if>
                    <#if attribute.annotations.inputHelperTextAfter??>
                        <div class="${properties.kcInputHelperTextAfterClass!} form-text text-muted mt-1" id="form-help-text-after-${attribute.name}" aria-live="polite">
...
<#macro inputTag attribute value>
    <input type="<@inputTagType attribute=attribute/>" 
           ...
           name="${attribute.name}" 
           value="${(value!'')}" 
           class="form-control <#if messagesPerField.existsError('${attribute.name}')>is-invalid</#if>"
...
<#macro selectTag attribute>
    <select id="${attribute.name}" 
            name="${attribute.name}" 
            class="form-select <#if messagesPerField.existsError('${attribute.name}')>is-invalid</#if>"
            aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
            <#if attribute.readOnly>disabled</#if>
    ...
    >
        ...
        <#list options as option>
            <option value="${option}" <#if attribute.values?seq_contains(option)>selected</#if>>
...
<#macro inputTagSelects attribute>
    <#if attribute.annotations.inputType=='select-radiobuttons'>
        ...
        <#assign classDiv='form-check'>
        <#assign classInput='form-check-input'>
        <#assign classLabel='form-check-label'>
    <#else>
    ...
    <#list options as option>
        <div class="${classDiv}">
            <input type="${inputType}" 
                   ...
                   name="${attribute.name}" 
                   value="${option}" 
                   class="${classInput} <#if messagesPerField.existsError('${attribute.name}')>is-invalid</#if>"
                   aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
                   <#if attribute.readOnly>disabled</#if>
                   <#if attribute.values?seq_contains(option)>checked</#if>
            />
            <label for="${attribute.name}-${option}" class="${classLabel} <#if attribute.readOnly>text-muted</#if>">
                <@selectOptionLabelText attribute=attribute option=option/>
            </label>
        </div>
    </#list>
